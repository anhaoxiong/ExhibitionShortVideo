...
@property (assign, nonatomic) BOOL fileEndDecoding;
...
- (void)clickNextButton:(UIButton *)button {
    ...
    if ([self.mixRecorder getTotalDuration] < minDuration) {
...
- (void)setupMixRecorder {
    ...
    videoConfiguration.videoSize = CGSizeMake(1080, 960);
    videoConfiguration.averageVideoBitRate = [QNBaseViewController suitableVideoBitrateWithSize:videoConfiguration.videoSize];
    // 设置相机采集在合拍中的位置
    videoConfiguration.cameraVideoFrame = CGRectMake(0, 0, 540, 960);
    // 设置素材视频在合拍中的位置
    videoConfiguration.sampleVideoFrame = CGRectMake(540, 0, 540, 960);
    ...
    // 如果素材视频没有音频通道，则使用 麦克风采集的音频作为最终生成的视频文件的音频

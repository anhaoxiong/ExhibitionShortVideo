...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.selectedBackgroundView.layer.borderColor = [UIColor colorWithRed:.8 green:.1 blue:.1 alpha:1].CGColor;
...
- (instancetype)initWithThumbImage:(NSArray<UIImage *> *)thumbArray videoDuration:(CMTime)duration {
    ...
    if (self) {
        ...
        self.currentTimeLabel.text = @"滑动选择音乐插入点";
...
- (void)setPlayingTime:(CMTime)time {
    ...
    self.currentTimeLabel.text = [NSString stringWithFormat:@"滑动选择音乐插入点: %0.1fs", CMTimeGetSeconds(time)];
}

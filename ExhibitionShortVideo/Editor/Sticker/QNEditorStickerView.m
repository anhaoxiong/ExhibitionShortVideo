...
@implementation QNEditorStickerView
...
- (instancetype)initWithThumbImage:(NSArray<UIImage *> *)thumbArray videoDuration:(CMTime)duration {
    ...
    if (self) {
        ...
        self.currentTimeLabel.text = @"滑动选择 GIF 插入点";
...
- (void)setPlayingTime:(CMTime)time {
    ...
    self.currentTimeLabel.text = [NSString stringWithFormat:@"滑动选择动图插入点: %0.1fs", CMTimeGetSeconds(time)];
}

- (void)clickDoneButton:(UIButton *)button {
    [self.delegate editorStickerViewDoneButtonClick:self];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    [self.delegate editorStickerView:self wantSeekPlayerTo:time];
    self.currentTimeLabel.text = [NSString stringWithFormat:@"滑动选择动图插入点: %0.1fs", CMTimeGetSeconds(time)];
    self.currentTime = time;
}
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    if (scrollView != self.timelinwCollectionView) return;
    self.isDragging = YES;
    [self.delegate editorStickerViewWillBeginDragging:self];
}
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    self.isDragging = NO;
    [self.delegate editorStickerViewWillEndDragging:self];
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (collectionView == self.gifSourceCollectionView) {
        ...
        [self.delegate editorStickerView:self addGifSticker:stickerModel];
    }
}
    ...
    for (QNStickerModel *stickerModel in self.addedStickerModelArray) {
        if (stickerModel.colorView == gestureView) {
            if (stickerModel != self.currentEditingSticker) {
                [self.delegate editorStickerView:self wantEntryEditing:stickerModel];
            }
            break;
        }
    }
...
-(void)setupPanBorder{
    ...
    self.topLineView.backgroundColor  = [UIColor redColor];
